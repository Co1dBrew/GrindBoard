...
router.get("/", async (req, res) => {
  try {
    ...
    const questions = await db
      ...
      .find({ _id: { $in: qIds.map((id) => new ObjectId(id)) } })
      .toArray();
...
router.get("/stats", async (req, res) => {
  try {
    ...
    const avgTime =
      totalSessions > 0
        ? (
          sessions.reduce((sum, s) => sum + s.timeSpent, 0) / totalSessions
        ).toFixed(1)
        : 0;
    ...
    for (const s of sessions) {
      ...
      for (const t of q.topic || []) {
        if (!topicMap[t]) topicMap[t] = { total: 0, solved: 0, time: 0 };
        topicMap[t].total++;
    ...
    res.json({ totalSessions, solved, unsolved, partial, avgTime, byTopic });
  } catch (err) {
    res.status(500).json({ error: err.message });
  }
});
...
router.get("/question/:questionId", async (req, res) => {
  try {
    ...
    const question = await db
      .collection("questions")
      .findOne({ _id: new ObjectId(req.params.questionId) });
...
router.post("/", async (req, res) => {
  try {
    ...
    const inserted = await db.collection("practice_sessions").insertOne(doc);
